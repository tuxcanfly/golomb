...
class Golomb {
  ...
  header(prev) {
    if (this.data.length === 0)
      return hash256.root(ZERO_HASH, prev);
  ...
  fromItems(P, key, items) {
    ...
    const n = this.m.mul(new U64(this.n));
    for (const item of items) {
      assert(Buffer.isBuffer(item));
      const hash = sipmod64(item, key, n);
      values.push(hash);
    }
  ...
  fromBytes(N, P, data) {
    ...
    this.n = N;
    this.p = P;
    this.m = U64.shift(this.n, this.p);
    this.data = data;
    ...
    for (let i = 0; i < block.txs.length; i++) {
      ...
      for (const output of tx.outputs) {
        ...
        if (output.script.raw[0] === 106 &&
            isPushOnly(output.script.code.slice(1)))
          continue;
